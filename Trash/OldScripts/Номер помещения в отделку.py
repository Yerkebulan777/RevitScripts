...
def GetWallfinishing(level, BBoxMin, BBoxMax):
    ...
    WallWidhtFilter = GetFilterDoubleParameter(BuiltInParameter.WALL_ATTR_WIDTH_PARAM, 0.15, FilterNumericLessOrEqual())
...
def GetRoomByCurve(room_doc, elevat, item):
    samples = 3
    room = None
    if "Curve" in item.Location.GetType().Name:
        ...
        fipts = flatten((GetFibonachiCirclePoints(p, 9) for p in points))
        pntZ = round((elevat + 5.0), 3)
        rooms = [room_doc.GetRoomAtPoint(XYZ(p.X, p.Y, pntZ)) for p in fipts]
        rooms = (room for room in rooms if room)
        if rooms and len(set(rooms)) == 1: return rooms[0]
        if rooms and len(set(rooms)) != 1:
            room = GetMostValue(rooms)
        else:
            fipts = flatten([GetFibonachiCirclePoints(p, 30) for p in points])
        rooms = [room_doc.GetRoomAtPoint(XYZ(p.X, p.Y, pntZ)) for p in fipts]
        rooms = (room for room in rooms if room)
        if rooms and len(set(rooms)) == 1: return rooms[0]
...
def SetRoomNumber(rooms, items, prm_name):
    if not items: pass
...
if rooms:
    ...
    elems = SetRoomNumber(rooms, items, prm_name)
...
OUT = test #result_quantity, undefd_quantity, undefined