...
def FilterByWorksetUserAndGroup(items):
    ...
    group_elems = flatten([i.GetMemberIds() for i in groups if i])
...
def GetMostValue(values):
    ...
    for val in set_common:
        if val == None and values.row(val) <= tolerance:
            pass
        elif val == None:
            continue
        count = values.row(val)
        if count > count_val:
            count_val = count
            most_val = val
    result = most_val
...
def SetValue(item, prm_name, value):
    if item == None: pass
    try:
        item.LookupParameter(prm_name).Set(value);
        return
    except:
        pass
...
if undefd_quantity == 0:
    ...
    elems_name = ShowElementName(undefined);
