...
from Autodesk.Revit.DB import \
    ElementId, XYZ, Line, Wall, WallType, FilteredElementCollector, BuiltInCategory, BuiltInParameter, \
    Transaction, FilterStringContains, FilterNumericLessOrEqual, FilterNumericGreater, \
...
def get_rayIntersection(line, view3D):
...
def Execute(self, commandData, message, elements):
    pp = DB.MessageDescriptionGatheringPreprocessor()
    with Transaction(doc, "CreateWallFinishing") as trx:
        ops = trx.GetFailureHandlingOptions()
        ops.SetFailuresPreprocessor(pp)
        trx.SetFailureHandlingOptions(ops)
        trx.Start("Marks")
        trx.Commit()
        pp.ShowDialogue()
...
if actiview.ViewType == ViewType.FloorPlan:
    ...
    with Transaction(doc, "CreateWallFinishing") as trx:
        ...
        double_rule = FilterDoubleRule(double_pvp, FilterNumericGreater(), 3.0, 0.5)
                ...
                for group in group_closed_borderlines(boundictionaries):
                    ...
                    for idx, sequence in enumerate(group):
                        ...
                        if lengthen and previous['type'].Id.IntegerValue == sequence['type'].Id.IntegerValue:
                            ...
                            if v1.CrossProduct(v2).IsAlmostEqualTo(XYZ.Zero, 0.005):
                                continue
                    ...
                    for idx, point in enumerate(points):
                        ...
                        line = Line.CreateBound(points[idx - 1], point)
                        if any(group): group.reverse()
                        joineds = []
                        for sequence in group:
                            center = sequence['workset_name'].Evaluate(0.5, True)
                        ...
                        # newline.LineStyle.GraphicsStyleCategory.LineColor = DB.Color(175, 50, 50)
                        # informs.add(workset_name.Convert())

        trx.Commit()

########## ПЕРЕД ТЕМ КАК СТРОИТЬ СТЕНЫ ПРОБИТЬ ЛУЧОМ НА РАССТОЯНИЕ ОКОЛО 0,5 ПОТОМ ОБОЙТИСЬ БЕЗ ПРОВЕРКИ
#######################################################################################################################
# selids = List[ElementId]()
# for element in selected: selids.Add(element.Id)
