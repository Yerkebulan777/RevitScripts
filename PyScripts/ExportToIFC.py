...
def adjust_export_links(doc, file_name):
    Output("\nStart adjust link files ...")
    collector = FilteredElementCollector(doc).OfClass(RevitLinkType)
    for link_type in collector.OfCategory(BuiltInCategory.OST_RvtLinks).ToElements():
        AttachTypeName = link_type.AttachmentType.ToString()
        if RevitLinkType.IsLoaded(doc, link_type.Id):

            if AttachTypeName == 'Attachment':
                ...
                Output("Attached linked file {0} is added to export {1}".format(link_name, contains))
                if not contains: link_type.Unload(None)

            if AttachTypeName == 'Overlay':
                try:
