...
def LogOutputErrorDetails(exception, output_, verbose=True):
    exceptionMessage = " error not defined "
    output = global_test_mode.PrefixedOutputForGlobalTestMode(output_, EXCEPTION_MESSAGE_HANDLER_PREFIX)
    if isinstance(exception, exceptions.Exception):
        exceptionMessage = str(exception.message)
    elif isinstance(exception, System.Exception):
        exceptionMessage = str(exception.message3)
    output("\nException: [" + type(exception).__name__ + "] " + exceptionMessage)
    try:
